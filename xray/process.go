...
func TestConfig(config *Config) error {
	if err := prepareLogDirectories(config); err != nil {
		return err
	}
	data, err := json.MarshalIndent(config, "", "  ")
	if err != nil {
		return common.NewErrorf("marshal xray config failed: %v", err)
	}
	tmpFile, err := os.CreateTemp("", "x-ui-xray-test-*.json")
	if err != nil {
		return common.NewErrorf("create temp config failed: %v", err)
	}
	defer func() {
	...
	if _, err = tmpFile.Write(data); err != nil {
		_ = tmpFile.Close()
		return common.NewErrorf("write temp config failed: %v", err)
	}
	_ = tmpFile.Close()
	...
	if err != nil {
		return common.NewErrorf("xray config test failed: %v, output: %s", err, string(output))
	}
	return nil
}
...
func prepareLogDirectories(config *Config) error {
	logConfig := bytes.TrimSpace(config.LogConfig)
	if len(logConfig) == 0 || bytes.Equal(logConfig, []byte("null")) {
		return nil
	}

	logPaths := &fileLogConfig{}
	if err := json.Unmarshal(logConfig, logPaths); err != nil {
		return common.NewErrorf("parse xray log config failed: %v", err)
	}

	for _, path := range []string{logPaths.Access, logPaths.Error} {
		if path == "" {
			continue
		}
		...
		if err := os.MkdirAll(dir, 0755); err != nil {
			return common.NewErrorf("create xray log directory <%v> failed: %v", dir, err)
		}
	}
	return nil
}
...
func (p *process) Start() (err error) {
	...
	err = prepareLogDirectories(p.config)
	if err != nil {
		return err
	}
